  ...
  const crearIconoLluvia = (r, cr, tipo = 'pluviometro', sinDatos = false, nombre = '') => {
    ...
    return divIcon({
      ...
      html: `<div class="${clase}" style="display:flex;flex-direction:column;align-items:center;margin-top:-30px;opacity:${sinDatos ? 0.6 : 1}">
               <div style="background:${cr ? '#1e293b' : '#111827'};border:1px solid ${borde};color:${borde};font-size:10px;font-weight:700;padding:2px 7px;border-radius:4px;white-space:nowrap">${nombre ? `<span style="color:#e8f1fa">${nombre}</span><span style="opacity:.45"> · </span>` : ''}${sinDatos ? 'sin datos' : r.toFixed(1) + ' mm'}</div>
               <div style="font-size:22px;line-height:1;margin-top:2px">${esInnova ? '📡' : (esDavis ? '🌡️' : '🌧️')}</div>
             </div>`,
      iconSize: [160, 62], iconAnchor: [80, 45],
    });
  };
