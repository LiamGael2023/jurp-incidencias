  ...
  const crearIconoLluvia = (r, cr, tipo = 'pluviometro', sinDatos = false, nombre = '') => {
    ...
    return divIcon({
      ...
      html: `<div class="${clase}" style="display:flex;flex-direction:column;align-items:center;margin-top:-30px;opacity:${sinDatos ? 0.6 : 1}">
               ...
               <div style="line-height:0;margin-top:3px">${esInnova ? '<span style="font-size:22px;line-height:1">📡</span>' : (esDavis ? `<svg width="28" height="28" viewBox="0 0 28 28" style="display:block;filter:drop-shadow(0 1px 2px rgba(0,0,0,.6))"><circle cx="14" cy="14" r="12.2" fill="${cr ? '#1e293b' : '#111827'}" stroke="${borde}" stroke-width="1.8"/><g stroke="${borde}" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" fill="none"><path d="M14 9.6 V19"/><path d="M9.6 11 H18.4"/><path d="M10.4 22.4 L14 19 L17.6 22.4"/></g><g fill="${borde}"><circle cx="9.4" cy="11" r="1.7"/><circle cx="18.6" cy="11" r="1.7"/><circle cx="14" cy="8" r="1.9"/></g></svg>` : '<span style="font-size:22px;line-height:1">🌧️</span>')}</div>
             </div>`,
