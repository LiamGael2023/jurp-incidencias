  ...
  const crearIconoLluvia = (r, cr, tipo = 'pluviometro', sinDatos = false, nombre = '') => {
    ...
    return divIcon({
      ...
      html: `<div class="${clase}" style="display:flex;flex-direction:column;align-items:center;margin-top:-30px;opacity:${sinDatos ? 0.6 : 1}">
               ...
               <div style="line-height:0;margin-top:3px">${esInnova ? '<span style="font-size:22px;line-height:1">📡</span>' : (esDavis ? `<svg width="30" height="32" viewBox="0 0 28 30" style="display:block;filter:drop-shadow(0 1px 2px rgba(0,0,0,.55))"><g fill="${borde}" stroke="#0b1220" stroke-width="1.6" stroke-linejoin="round" paint-order="stroke"><rect x="12.7" y="5" width="2.6" height="20" rx="0.6"/><rect x="5.5" y="6.1" width="17" height="1.9" rx="0.9"/><circle cx="5.6" cy="7" r="2.3"/><circle cx="14" cy="3.6" r="2.3"/><circle cx="22.4" cy="7" r="2.3"/><rect x="16.4" y="11.2" width="5.2" height="7" rx="2.2"/><path d="M8.6 28.6 L12.4 24.4 h3.2 l3.8 4.2 z"/></g></svg>` : '<span style="font-size:22px;line-height:1">🌧️</span>')}</div>
             </div>`,
                  ...
                  <div key={i} className="gis-resultado" onClick={() => seleccionarResultado(r)}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <b>
                        {r.name}
                        {r.origen === 'inventario' && (
                          <span className={`gis-res-inv amb-${r.ambito || 'SIN_DATO'}`}>
                            {r.ambito === 'JURP' ? 'JURP'
                              : r.ambito === 'PECH' ? 'Chavimochic'
                              : 'inventario'}
                          </span>
                        )}
                      </b>
